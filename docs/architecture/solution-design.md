...
A skill `delivery-orchestrator` coordena somente os agentes especializados já
instalados e aplicáveis à entrega; sua primeira capacidade é a entrega separada
da documentação de aprendizagem. As integrações com agentes posteriores são
adicionadas somente depois de cada um estar validado e integrado à `main`.
A skill `technical-documenter` mantém em `templates/` os modelos de ADR, runbook
e guia de migração, usados somente na criação do respectivo tipo documental.
Os arquivos em `.codex/agents/` definem os oito perfis especializados do Codex;
...
| `.agents/skills/learning-documenter/assets/`            | template canônico da documentação de aprendizagem de módulos integrados        |
| `.agents/skills/delivery-orchestrator/`                 | sequência e gates de entrega dos agentes já disponíveis                         |
| `.agents/skills/technical-documenter/templates/`        | templates de ADR, runbook e guia de migração quando aplicáveis                 |
| `.codex/agents/`                                        | perfis dos oito agentes no OpenAI Codex                                        |
| `.codex/agents/learning-documenter.toml`                | perfil responsável pela documentação de aprendizagem de módulos integrados     |
| `.codex/agents/delivery-orchestrator.toml`              | perfil que coordena etapas de entrega usando apenas agentes já disponíveis     |
...
### 4.7 Orquestração, construção, documentação e revisão dos módulos por agentes Codex

O `delivery-orchestrator` coordena a sequência de entrega, sem assumir as
responsabilidades especializadas dos demais agentes nem contornar os controles
do repositório. Ele começa após a implantação do `learning-documenter`, para
orquestrar a entrega da documentação de aprendizagem do próprio agente. Nessa
primeira etapa, usa somente o `learning-documenter`; não exige a existência de
construtor, documentador técnico ou revisor. Cada integração posterior é
adicionada somente após o respectivo agente estar validado e integrado à `main`.
O orquestrador mantém os gates de Issue, branch, Pull Request e CI e requer
autorização explícita para ações remotas que a governança do repositório não
delegar automaticamente, em especial o merge.

Enquanto o Pull Request de implementação estiver aberto, cada módulo será
conduzido por três agentes Codex com responsabilidades separadas:
...
- `technical-documenter` cria, atualiza ou revisa, no mesmo Pull Request, somente
  ...
  e não cria documentação de aprendizagem. Ao criar uma ADR, um runbook ou um
  guia de migração, usa o template correspondente; atualizações preservam o
  formato canônico existente;
...
Depois do merge na `main`, o `delivery-orchestrator` inicia ou retoma a Issue,
branch e Pull Request próprios da documentação de aprendizagem e aciona o
`learning-documenter`. Este confirma que todos os Pull Requests que compõem o
módulo estão com estado `MERGED`, consulta as evidências da entrega integrada e
cria ou atualiza exatamente um documento canônico em
`docs/learning/modules/`. Esse documento usa
`assets/module-learning.md` como contrato estrutural e não altera a
implementação nem a documentação técnica da entrega.
